...
public class FnoController : ControllerBase
{
    ...
    public IActionResult GetChart(string symbol, [FromQuery] string type, [FromQuery] decimal strike, [FromQuery] string expiry)
    {
        ...
        var spotPrice = instrument.BasePrice;
        var optionPrice = type == "CE" 
            ? Math.Max(5, spotPrice - strike + (decimal)_rng.NextDouble() * 100)
            : Math.Max(5, strike - spotPrice + (decimal)_rng.NextDouble() * 100);
        ...
        for (int i = 0; i < 72; i++) // 6 hours of 5-min candles
        {
            ...
            var change = (decimal)(_rng.NextDouble() - 0.48) * optionPrice * 0.02m;
            var close = Math.Max(1, open + change);
            var high = Math.Max(open, close) + (decimal)_rng.NextDouble() * optionPrice * 0.005m;
            var low = Math.Min(open, close) - (decimal)_rng.NextDouble() * optionPrice * 0.005m;
        ...
        for (int i = 10; i < 72; i += _rng.Next(8, 20))
        {
            signals.Add(new
            {
                time = startTime.AddMinutes(i * 5),
                action = _rng.Next(2) == 0 ? "BUY" : "SELL",
                price = currentPrice + (decimal)(_rng.NextDouble() - 0.5) * 10,
                label = _rng.Next(2) == 0 ? "Momentum Break" : "RSI Reversal"
            });
        }

        var support = Math.Round(optionPrice * 0.85m, 2);
        var resistance = Math.Round(optionPrice * 1.2m, 2);

        return Ok(new
        {
            success = true,
            data = new { candles, signals, supportLevel = support, resistanceLevel = resistance },
            timestamp = DateTime.UtcNow
        });
    }
    ...
    private object GenerateOption(string symbol, decimal strike, string optionType, decimal spot, string expiry)
    {
        ...
        var timeValue = (decimal)(_rng.NextDouble() * 50 + 5);
        var premium = Math.Round(intrinsic + timeValue, 2);
        var change = Math.Round((decimal)(_rng.NextDouble() - 0.45) * premium * 0.1m, 2);
        var iv = Math.Round(10 + (decimal)_rng.NextDouble() * 30, 1);
        ...
        return new
        {
            ...
            bidPrice = Math.Max(0.05m, premium - 0.5m),
            askPrice = premium + 0.5m,
            signal = signal.action,
            signalConfidence = signal.confidence,
